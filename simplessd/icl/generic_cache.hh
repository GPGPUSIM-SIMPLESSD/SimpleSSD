...
class GenericCache : public AbstractCache {
 private:
  ...
  struct {
    uint64_t request[2];
    uint64_t cache[2];
    uint64_t cachelines_read;
    uint64_t cachelines_write;
    uint64_t cache_flush;
    uint64_t cache_read_miss;
    uint64_t cache_write_miss;
    uint64_t cache_read_hit;
    uint64_t cache_write_hit;
